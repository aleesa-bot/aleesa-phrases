...
sub Seed () {
	...
	while (readline $fh) {
		...
		chomp();
		my ($phrase, $days) = split / \|\| /;  ## no critic (RegularExpressions::RequireDotMatchAnything), do you see ANY dots here?
		my @daylist = split(//, $days);  ## no critic (RegularExpressions::RequireDotMatchAnything), do you see ANY dots here?
		...
		foreach my $day (@daylist) {
			...
			unless ($day =~ /^\d$/) {  ## no critic (RegularExpressions::RequireDotMatchAnything), do you see ANY dots here?
				next;
			}
